...
import { registerDefaultUserSchema } from "../../schemas/Schemes";
...
export default function DefaultUser() {
  const { userTypeData } = useCheckUserType();
  ...
  }Authentication/ClientRegister`;

  const { handleSubmit, isLoading, isError, errorList, userExistError } =
    useRegister(user, registerDefaultUserSchema, apiUrl);
  ...
  return (
    <div className="container border-top">
      <form className="row g-3 mt-2" onSubmit={handleSubmit}>
        ...
        <div className="col-md-3">
          <label className="form-label text-dark">المحافظة</label>
          <select
            className="form-control border new-inputs"
            name="governorate"
        ...
        <div className="col-md-6">
          ...
          >
            <option>اختر نوع المؤسسه</option>

            {userTypeData[2]?.subTypes?.map((place, index) => (
